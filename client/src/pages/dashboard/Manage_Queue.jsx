...
import '../../index.css';
import { ChevronDown, ChevronUp, Monitor, Megaphone, Bell, X, Pause, SkipForward, Check } from 'lucide-react';
...
export default function Manage_Queue() {
  ...
  const [currentQueue, setCurrentQueue] = useState({
    queueNo: 'R001',
    type: 'Regular',
    name: 'Laroco, Jan Lorenz',
    studentId: '2312457',
    course: 'BSIT - 3rd Year',
    time: '9:01 AM',
    requests: [
      { id: 1, name: 'Good Moral Certificate', status: 'In Progress' },
      { id: 2, name: 'Insurance Payment', status: 'In Progress' },
      { id: 3, name: 'Temporary Gate Pass', status: 'In Progress' },
      { id: 4, name: 'Temporary Gate Pass', status: 'In Progress' }
    ]
  });

  const [deferredQueue, setDeferredQueue] = useState([
    { queueNo: 'R009', studentId: '23921845', name: 'John Doe', course: 'BSHM- 1st Year', type: 'Regular', time: '11:21 AM', requests: [
      { id: 1, name: 'Transmittal Letter', status: 'Stalled' },
      { id: 2, name: 'Good Moral Certificate', status: 'Stalled' }
    ]},
    { queueNo: 'R020', studentId: '23219823', name: 'Kevin Durant', course: 'BSIT - 2nd Year', type: 'Regular', time: '9:05 AM', requests: [
      { id: 1, name: 'Insurance Payment', status: 'Stalled' }
    ]},
    { queueNo: 'R021', studentId: '2323003', name: 'Stephen Curry', course: 'BSA - 3rd Year', type: 'Regular', time: '9:07 AM', requests: [
      { id: 1, name: 'Temporary Gate Pass', status: 'Stalled' },
      { id: 2, name: 'Good Moral Certificate', status: 'Stalled' },
      { id: 3, name: 'Insurance Payment', status: 'Stalled' }
    ]},
    { queueNo: 'R022', studentId: '23844352', name: 'Lebron James', course: 'BSCS - 4th Year', type: 'Regular', time: '9:07 AM', requests: [
      { id: 1, name: 'Insurance Payment', status: 'Stalled' }
    ]},
    { queueNo: 'R023', studentId: '23844362', name: 'Dwayne Wade', course: 'BSN - 2nd Year', type: 'Regular', time: '9:08 AM', requests: [
      { id: 1, name: 'Transmittal Letter', status: 'Stalled' }
    ]},
      { queueNo: 'R022', studentId: '23844352', name: 'Lebron James', course: 'BSCS - 4th Year', type: 'Regular', time: '9:07 AM', requests: [
      { id: 1, name: 'Insurance Payment', status: 'Stalled' }
  ...
  const [nextInLine, setNextInLine] = useState([
    { queueNo: 'P005', studentId: '23219652', name: 'John Doe', course: 'BSIT - 4th Year', type: 'Priority', request: 'Good Moral Certificate', time: '9:06 AM' },
    { queueNo: 'R020', studentId: '23219823', name: 'Kevin Durant', course: 'BSA - 2nd Year', type: 'Regular', requests: ['Insurance Payment', 'Transmittal Letter', 'Good Moral Certificate'], time: '9:05 AM' },
    { queueNo: 'R021', studentId: '2323003', name: 'Stephen Curry', course: 'BSCS - 3rd Year', type: 'Regular', request: 'Temporary Gate Pass',  time: '9:07 AM' },
    { queueNo: 'R022', studentId: '23844352', name: 'Lebron James', course: 'BSN - 1st Year', type: 'Regular', request: 'Insurance Payment', time: '9:07 AM' },
    { queueNo: 'R023', studentId: '23844363', name: 'Chris Paul', course: 'BSHM - 2nd Year', type: 'Regular', request: 'Transmittal Letter', time: '9:08 AM' },
    { queueNo: 'R024', studentId: '23844364', name: 'James Harden', course: 'BSIT - 3rd Year', type: 'Priority', request: 'Good Moral Certificate', time: '9:09 AM' },
    { queueNo: 'R022', studentId: '23844352', name: 'Lebron James', course: 'BSN - 1st Year', type: 'Regular', request: 'Insurance Payment', time: '9:07 AM' },
    { queueNo: 'R023', studentId: '23844363', name: 'Chris Paul', course: 'BSHM - 2nd Year', type: 'Regular', request: 'Transmittal Letter', time: '9:08 AM' },
    { queueNo: 'R024', studentId: '23844364', name: 'James Harden', course: 'BSIT - 3rd Year', type: 'Regular', request: 'Good Moral Certificate', time: '9:09 AM' }
  ]);

  const filteredNextInLine = nextInLine.filter(item => 
    item.queueNo.toLowerCase().includes(searchTerm.toLowerCase()) ||
    item.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
    item.studentId.includes(searchTerm)
  );

  const filteredDeferredQueue = deferredQueue.filter(item =>
    item.queueNo.toLowerCase().includes(deferredSearchTerm.toLowerCase()) ||
    item.name.toLowerCase().includes(deferredSearchTerm.toLowerCase()) ||
    item.studentId.includes(deferredSearchTerm) 
  );
  ...
  const handleDeferredAction = (requestId, action) => {
    ...
    setDeferredQueue(prev => prev.map(queue => {
  ...
  const handleCallNext = () => {
    ...
    const nextPerson = nextInLine[0];
    
    let requests = [];
    if (Array.isArray(nextPerson.requests)) {
      requests = nextPerson.requests.map((req, index) => ({
        id: index + 1,
        name: req,
        status: 'In Progress'
      }));
    } else if (nextPerson.request) {
      requests = [{
        id: 1,
        name: nextPerson.request,
        status: 'In Progress'
      }];
      if (nextPerson.count) {
        for (let i = 1; i <= nextPerson.count; i++) {
          requests.push({
            id: i + 1,
            name: `Additional Request ${i}`,
            status: 'In Progress'
          });
        }
      }
  ...
  <div className="min-h-screen bg-transparent w-full p-4 md:p-10">
      ...
      <h1 className="text-2xl md:text-3xl font-semibold text-left text-gray-900 mb-9 mt-6">Manage Queue</h1>
  ...
  {/* container */}
        <div className="flex  items-center justify-between gap-6 h-full">
          ...
          <div className="border-2 flex-1  border-[#E2E3E4] rounded-lg p-6 h-full">
            <div className=" text-left mb-4 ">
              <div className="text-7xl text-center border border-[#1A73E8] rounded-xl py-4 font-bold text-[#1A73E8] mb-2">{currentQueue.queueNo}</div>
              ...
              <div className="space-y-3 text-sm text-left">
                <div>
                  ...
                  <div className="font-semibold text-[#202124]">{currentQueue.name}</div>
                </div>
                <div>
                  <div className="text-[#686969] mb-1">Student ID</div>
                  <div className="font-semibold text-[#202124]">{currentQueue.studentId}</div>
                </div>
                <div>
                  <div className="text-[#686969] mb-1">Course & Year</div>
                  <div className="font-semibold text-[#202124]">{currentQueue.course}</div>
                </div>
                <div>
                  <div className="text-[#686969] mb-1">Time</div>
                  <div className="font-semibold text-[#202124]">{currentQueue.time}</div>
                </div>
              </div>
            </div>
                        ...
                        <table className="w-full">
                          <thead className="bg-white sticky top-0 z-10">
                            ...
                            <tr className="border-b border-gray-200">
                              <th colSpan="3" className="text-left py-5 px-4 text-xl font-medium  text-gray-900 ">
                                Service Requests
                              </th>
                            </tr>
                            {/* Table Headers */}
                            <tr className="border-b border-gray-200">
                              <th className="text-left py-3 px-4 text-sm font-semibold text-[#686969] w-44">Request</th>
                              <th className="text-left py-3 px-4 text-sm font-semibold text-[#686969] w-36">Status</th>
                              <th className="text-center py-3 px-4 text-sm font-semibold text-[#686969] w-16">Actions</th>
                            </tr>
                          </thead>
                          <tbody>
                              ...
                              <td className="text-center py-3 px-4">
                                <div className="flex gap-2 items-center justify-center">
                                  ...
                                  <div className="relative group">
                                    <button
                                      onClick={() => handleRequestAction(request.id, 'done')}
                                  ...
                                  <div className="relative group">
                                    <button
                                      onClick={() => handleRequestAction(request.id, 'stall')}
      ...
      <div className="bg-white rounded-xl shadow-xs mb-4 overflow-hidden">
          ...
          >
            ...
            {deferredOpen ? <ChevronUp className="w-5 h-5 text-gray-600" /> : <ChevronDown className="w-5 h-5 text-gray-600" />}
          </button>
                  ...
                  <table className="w-full min-w-[700px]">
                    <thead className="bg-white sticky top-0 z-50">
                      <tr className="border-b border-[#E2E3E4]">
                        <th className="text-left py-3 px-4 text-sm font-semibold text-[#686969] w-40">Student ID</th>
                        <th className="text-left py-3 px-4 text-sm font-semibold text-[#686969] w-48">Name</th>
                        <th className="text-left py-3 px-4 text-sm font-semibold text-[#686969] w-64">Request</th>
                        <th className="text-left py-3 px-4 text-sm font-semibold text-[#686969] w-32">Action</th>
                      </tr>
                    </thead>
                    ...
                    <tbody>
                      {filteredDeferredQueue.length > 0 ? (
                        filteredDeferredQueue.map((item, index) => (
                          <tr key={index} className="border-b border-[#E2E3E4] hover:bg-gray-50">
                            <td className="text-left py-3 px-4 text-sm text-[#202124] w-40 ">{item.studentId}</td>
                            <td className="text-left py-3 px-4 text-sm text-[#202124]  w-48">{item.name}</td>
                              ...
                              <div 
                                ...
                                >
                                  ...
                                  {item.requests.length > 1 && (
                                    <>
                                      <span className="ml-2 bg-transparent font-semibold border-1 border-[#1A73E8]  text-[#1A73E8] text-xs px-2 py-0.5 rounded-full cursor-pointer"
                                            onMouseEnter={() => setHoveredRow(`deferred-${index}`)}
                                      ...
                                      {hoveredRow === `deferred-${index}` && (
                                        <div className="absolute bottom-full left-0 mb-2 border border-[#E2E3E4] bg-white text-black p-3 rounded-lg shadow-lg z-50 min-w-[200px]">
                                          {item.requests.slice(1).map((req, idx) => (
        ...
        <div className="bg-white rounded-xl shadow-xs overflow-hidden">
          ...
          >
            <div className="flex items-center gap-3">
              <span className="font-semibold text-gray-900">Next in Line</span>
              <span className="bg-red-500 text-white text-xs font-semibold px-2 py-1 rounded-full min-w-[24px] text-center">
                {nextInLine.length}
              </span>
            </div>
            {nextInLineOpen ? <ChevronUp className="w-5 h-5 text-gray-600" /> : <ChevronDown className="w-5 h-5 text-gray-600" />}
          </button>
                ...
                <div className="overflow-y-scroll custom-scrollbar max-h-96">
                  <table className="w-full min-w-[700px] ">
                    <thead className=" sticky top-0 bg-white z-50 ">
                      <tr className="border-b border-[#E2E3E4]">
                        <th className="text-left py-3 px-4 text-sm font-semibold text-[#686969] w-32">Queue No.</th>
                        <th className="text-left py-3 px-4 text-sm font-semibold text-[#686969] w-40">Student ID</th>
                        <th className="text-left py-3 px-4 text-sm font-semibold text-[#686969] w-48">Name</th>
                        <th className="text-left py-3 px-4 text-sm font-semibold text-[#686969] w-64">Request</th>
                        <th className="text-left py-3 px-4 text-sm font-semibold text-[#686969] w-32">Time</th>
                      </tr>
                    </thead>
                    <tbody >
                      {filteredNextInLine.length > 0 ? (
                      filteredNextInLine.map((item, index) => (
                        <tr key={index} className="border-b border-[#E2E3E4] hover:bg-gray-50 transition">
                          <td className="text-left py-4 px-4 text-sm font-semibold w-32">
                            <span className={item.type === 'Priority' ? 'text-[#F9AB00]' : 'text-[#1A73E8]'}>
                              {item.queueNo}
                            </span>
                          </td>
                          <td className="text-left py-4 px-4 text-sm text-[#202124] w-40">{item.studentId}</td>
                          <td className="text-left py-4 px-4 text-sm text-[#202124] w-48">{item.name}</td>
                          <td className="text-left py-4 px-4 text-sm text-[#202124] w-64">
                            {item.requests ? (
                               ...
                               {item.requests[0]}
                                {item.requests.length > 1 && (
                                  <>
                                    <span className="ml-2 bg-transparent text-[#1A73E8]  font-semibold border-1 border-[#1A73E8] text-xs px-2 py-0.5 rounded-full cursor-pointer"
                          ...
                          <td className="text-left py-3 px-4 text-sm text-gray-900">{item.time}</td>
                        </tr>
                      )) 
                      ) : (
                        <tr>
          ...
          <div className="fixed inset-0 bg-black/20 backdrop-blur-sm bg-opacity-50 flex items-center justify-center p-4 z-50">
            <div className="bg-white rounded-lg shadow-xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
              <div className="flex items-center justify-between p-6  border-gray-200 sticky top-0 bg-white z-10">
                <h2 className="text-xl font-semibold text-gray-900">Action Panel</h2>
                <button 
                  ...
                  <div className="border-2 flex-1 border-[#E2E3E4] rounded-lg p-6 h-full">
                    ...
                    <div className="space-y-3 text-sm text-left">
                      <div>
                        ...
                        <div className="font-semibold text-gray-900">{selectedQueue.name}</div>
                      </div>
                      <div>
                        <div className="text-gray-500 mb-1">Student ID</div>
                        <div className="font-semibold text-gray-900">{selectedQueue.studentId}</div>
                      </div>
                      <div>
                        <div className="text-gray-500 mb-1">Course & Year</div>
                        <div className="font-semibold text-gray-900">{selectedQueue.course}</div>
                      </div>
                      <div>
                        <div className="text-gray-500 mb-1">Time</div>
                        <div className="font-semibold text-gray-900">{selectedQueue.time}</div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex flex-col flex-5 justify-between h-full">
                          ...
                          <div className="h-75 overflow-y-auto custom-scrollbar">
                            <table className="w-full">
                              <thead className="bg-white sticky top-0 z-10">
                                ...
                                {/* Table Headers */}
                                <tr className="border-b border-gray-200">
                                  <th className="text-left py-3 px-4 text-sm font-semibold text-gray-700 w-44">Request</th>
                                  <th className="text-left py-3 px-4 text-sm font-semibold text-gray-700 w-36">Status</th>
                                  <th className="text-center py-3 px-4 text-sm font-semibold text-gray-700 w-16">Actions</th>
                                </tr>
                              </thead>
                              <tbody>
                                {selectedQueue.requests.map((request) => (
                                  ...
                                  <div className="relative group">
                                    <button
                                      onClick={() => handleDeferredAction(request.id, 'done')}
                                  ...
                                  <div className="relative group">
                                    <button
                                      onClick={() => handleDeferredAction(request.id, 'stall')}
                                  ...
                                  <div className="relative group">
                                    <button
                                      onClick={() => handleDeferredAction(request.id, 'skip')}
                                  ...
                                  <div className="relative group">
                                    <button
                                      onClick={() => handleDeferredAction(request.id, 'cancel')}
                    ...
                    <div className="flex gap-3 mt-8 justify-end">
                      <button 
                        ...
                        disabled={selectedQueue.requests.some(request => request.status === 'Stalled')}
                        className={`flex items-center gap-2 px-6 py-3 rounded-lg transition-colors ${
                          selectedQueue.requests.some(request => request.status === 'Stalled') 
                            ? 'bg-[#1A73E8]/50 text-gray-200 cursor-not-allowed' 
                            : 'bg-[#1A73E8] text-white hover:bg-blue-600 cursor-pointer'
                        }`}
                      >
