...
import { InlineLoading } from "../../components/InLineLoader.jsx"; // Import your inline loader
import {
...
export default function Manage_Queue() {
  ...
  const [assigningWindowId, setAssigningWindowId] = useState(null);
  const isAnyWindowAssigning = assigningWindowId !== null;
  ...
  // NEW: Updated useEffect for loading sequence
  useEffect(() => {
    const restoreOrLoad = async () => {
      if (!socket || !isConnected) return;

      // Stage 1: Page setup complete, show skeleton
      ...
      try {
        ...
        if (currentAssignment?.success && currentAssignment.assignment) {
          ...
          try {
            ...
            // Stage 3: Data loaded, hide skeleton
            setIsDataLoading(false);
  ...
  const handleWindowSelect = async (windowId) => {
    if (assigningWindowId) return; // Prevent multiple clicks

    unlockSpeech();
    setAssigningWindowId(windowId); // Set the specific window being assigned
    try {
      ...
      if (response?.success) {
        ...
        try {
          ...
          console.error("Failed to check current queue:", err);
          setCurrentQueue(getDefaultQueue());
        } finally {
          setIsDataLoading(false);
          setIsInitialDataLoaded(true);
        ...
        showToast("Window already taken. Refreshing...", "warning");
        await loadWindows();
      } else {
  ...
  }, [isDataLoading, globalQueueList.length, selectedWindow, currentQueue]);
  ...
  const windowButtons = availableWindows.map((window) => {
    const isThisWindowAssigning = assigningWindowId === window.id;
    return {
      text: window.name,
      ...
      className:
        window.status === "inactive"
          ...
          : isThisWindowAssigning
          ? "bg-[#1A73E8]/50 text-white/70 cursor-not-allowed w-full"
          : "bg-[#1A73E8] text-white hover:bg-[#1557B0] w-full",
      disabled: window.status === "inactive" || assigningWindowId !== null, // Disable all during any assignment
      // OR for more granular: disabled: window.status === "inactive" || isThisWindowAssigning,
    };
  });
  ...
  const handleScroll = (e) => {
    ...
    if (nearBottom && hasMoreWaiting && !isDataLoading) {
  ...
  return (
    <div className="relative min-h-screen w-full">
      {/* Window Selection Modal - Always on top of everything */}
      {showWindowModal && (
        <DynamicModal
          isOpen={showWindowModal}
          onClose={isAnyWindowAssigning ? undefined : handleCloseWindowSelect}
          title={
            isAnyWindowAssigning
              ? "Assigning Window..."
              : "Select a Window to Manage"
          }
          description={
            isAnyWindowAssigning
              ? "Please wait while we assign your window..."
              : "Please choose which service window you would like to manage."
          }
          ...
          modalWidth="w-1/3"
          modalHeight="h-auto"
          showCloseButton={!isAnyWindowAssigning}
        />
      )}
      {/* Stage 2: Show skeleton while data is loading, even if modal is open */}
      {isDataLoading && !isInitialDataLoaded && (
        <LoadingSkeleton
      ...
      {/* Data Loading Overlay for subsequent updates */}
      {isDataLoading && isInitialDataLoaded && (
        <div className="absolute inset-0 bg-white/50 backdrop-blur-sm flex items-center justify-center z-10 rounded-2xl">
          <InlineLoading
            text="Updating queue data..."
            isVisible={true}
            size="large"
          />
        </div>
      )}
      {(isInitialDataLoaded || showWindowModal) && currentQueue && (
        <div
          className={`min-h-screen bg-transparent w-full pr-3 pt-9 lg:pr-7 md:pl-15 xl:pl-9 xl:pt-11 xl:pr-7 pb-9 ${
            isDataLoading ? "opacity-50" : "opacity-100"
